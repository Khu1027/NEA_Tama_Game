...
def decrease_and_penalty(status_countdown):
    ...
    # decreasing status'
    while Main_Game_Variables.hunger_action.stat != 0 or hunger != 0:
        Main_Game_Variables.hunger_action.stat -= 1
        hunger -= 1
    while Main_Game_Variables.happiness_action.stat != 0 or happiness != 0:
        Main_Game_Variables.happiness_action.stat -= 1
        happiness -= 1
    while Main_Game_Variables.health.stat != 0 or health != 0:
        Main_Game_Variables.health_action.stat -= 1
        health -= 1
    # giving penalties
    while hunger != 0:
        Main_Game_Variables.hunger_action.penalty += 1
        hunger -= 1
    while happiness != 0:
        Main_Game_Variables.happiness_action.penalty += 1
        happiness -= 1
    while health != 0:
        Main_Game_Variables.health_action.penalty += 1
        health -= 1
...
if continue_game:
    ...
    if 1 > current_day:
        ...
        if Main_Game_Variables.pet.collective_stage != "Child":
            countdowns = evolution_countdowns(1, total_seconds, game_on_time, hunger_count, happiness_count, health_count)
            before_evo = [countdowns[0], countdowns[1], countdowns[2]]
            after_evo = [countdowns[3], countdowns[4], countdowns[5]]
            ...
            Main_Game_Variables.pet.change_stage = True
            # Code for playing the evolution animation
            Main_Game_Variables.pet.stage = "Child"
            Game_Files.evolution = "Child"
            if current_day == 3:
                Main_Game_Variables.pet.mortal = True
            else:
                Main_Game_Variables.pet.mortal = False
        ...
        if Main_Game_Variables.pet.collective_stage != "Teenager":
        ...
        if Main_Game_Variables.pet.collective_stage != "Adult":
